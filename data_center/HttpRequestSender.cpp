...
BOOL CHttpRequestSender::Send(CHttpRequest& Request, AssyncNotification* an)
{
	// Copy parameters
	m_Request = Request;
	m_Assync = an;
	...
	Request.m_aOutTextFields = m_Request.m_aOutTextFields; 
	return TRUE;
}
...
{ 
    ...
    hSession = InternetOpen(_T("datafile"), INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
    if(hSession==NULL)
    {
    ...
    // Write text fields
    for(it=m_Request.m_aInTextFields.begin(); it!=m_Request.m_aInTextFields.end(); it++)
    {
	...
	m_Request.m_aOutTextFields[ L"response" ] = std::string( ( CHAR* )pBuffer ); 
...
BOOL CHttpRequestSender::WriteTextPart(HINTERNET hRequest, CString sName)
{
    ...
    std::map<CString, std::string>::iterator it = m_Request.m_aInTextFields.find(sName);
    if(it==m_Request.m_aInTextFields.end())
    {
